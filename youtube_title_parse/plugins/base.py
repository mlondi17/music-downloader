...
def clean_fluff(text):
    ...
    text = re.sub(r"\s*(of+icial\s*)?audio", "", text, flags=re.IGNORECASE)  # (official)? audio
    text = re.sub(r"\s*(original\s*)mix", "", text, flags=re.IGNORECASE)  # (original) mix
    text = re.sub(r"\s*(extended\s*)mix", "", text, flags=re.IGNORECASE)  # (extended) mix
